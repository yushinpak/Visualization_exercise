...
const sizeValue = d => d["Total Dead and Missing"]; // arrow function
...
const Bubbles = ({ 
    ...
    return (
        // Completed_TODO 2.2 wrap everything that follows in a group element with className bubbleMarks
            // Completed_TODO 2.2: enter a JS scope
                // Completed_TODO 2.2: map the data to get each row and index
                    // Completed_TODO 2.2: use the projection to get the x and y coordinates from the coords
        ...
        <g className="bubbleMarks">
            {data.map((row, index) => {
                const [x, y] = projection(row.coords); // Role: Converts geographic coordinates to screen coordinates.
                return (
                    <circle
                        className="bubble"
                        key={index}
                        cx={x}
                        cy={y}
                        r={sizeScale(sizeValue(row))} // calculated radius size
                    />
                );
            })}
        </g>
